...
Traffic entering into Customer Edge (CE1) router will be classified by matching source address , protocols and destination port (for ssh traffic) and subsequently will be assigned to appropriate forwarding class. This type of classifier is called "Multifield Classifier". Egress traffic from CE1 towards Provider Edge (PE) 1&2 routers will be marked with appropriate DSCP code points using rewrite-rule. Once traffic will enter ingress PE routers it will be classified by matching DSCP bits and traffic will be assigned to appropriate forwarding class. This type of classifier is called "Behavior Aggregate Classifier". Ingress PEs  will encapsulate packets in MPLS header and will forward it towards next LSR. As DSCP based classified packets will be not identified by MPLS label switch routers (LSR) and to solve this challenge we need to apply EXP based rewrite rule on ingress PEs egress interface.  Subsequently on ingress interface of each LSR traffic will be classified using MPLS header EXP bits. On each LSR egress EXP based  rewrite rule will be applied similar ingress PE egress interface rewrite rule.  Once traffic will reach egress PEs, MPLS label will be removed and IP packet will be forwarded to egress CE. On egress PE interfaces; connected with egress CE ,  DSCP based rewrite rules will be applied so that on egress CE ingress interface DSCP based Behavior Aggregate Classifiers can be applied. Eventually on egress CE traffic will exit to destination host via appropriate queue of egress interface. 
